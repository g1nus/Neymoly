    ...
    if(child_pid>0){//padre
        ...
        if(tmppipe==NULL){//se il pipe temporaneo non e' inizializzato ful dire che sono il padre principale(quello che deve eseguire l'ultimo comando)
            ...
            if(strcmp(cmd[y-2],"|")==0){
                ...
                printf(MAGENTA "[%i]<pipedrun:info> [FIRST ATHER] I'm an input file(%s), I don't have to do nothing\n", getpid(), cmd[y-1]);
            }
            ...
            if(strcmp(cmd[y-2],"|")==0){
                ...
                printf(MAGENTA "[%i]<pipedrun:info> [FATHER] I'm an input file(%s), I need to forward to the next command\n", getpid(), cmd[y-1]);
